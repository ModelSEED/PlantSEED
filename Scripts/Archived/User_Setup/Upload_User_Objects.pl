...
my @Dir_Contents = Bio::P3::Workspace::ScriptHelpers::wscall("ls",{ paths => ["/".$User], adminmode=>1 })->{"/".$User};
my $MakePS = 0;
...
if($MakePS){
    ...
    Bio::P3::Workspace::ScriptHelpers::wscall("create",{ objects => [["/".$User."/plantseed",'folder']], adminmode=>1, setowner=>$User, overwrite=>1});
    Bio::P3::Workspace::ScriptHelpers::wscall("create",{ objects => [[$User_Root,'folder']], adminmode=>1, setowner=>$User, overwrite=>1});
}
...
Bio::P3::Workspace::ScriptHelpers::wscall("create",{ objects => [[$User_Root.'/'.$Genome,"genome",{},$data]], overwrite => 1, adminmode => 1, setowner=>$User });
print "Genome uploaded\n";

#Creating Genome folder
#If it already exists, nothing changes
#Permissions set in top-level folder
Bio::P3::Workspace::ScriptHelpers::wscall("create",{ objects => [[$User_Root.'/.'.$Genome,"folder"]], adminmode=>1, setowner=>$User });
